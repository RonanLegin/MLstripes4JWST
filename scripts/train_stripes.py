...
def preprocess_batch(src_batch, noise, pink_template):
    ...
    template = get_bkgr(x_batch)

    # Remove background source from original image
    x_batch = x_batch - template
...
model = Unet(stacks, stackwidth, filters_base, kernel_size, save_path='weights_strp', **kw)    
model_bkgr = Unet(stacks, stackwidth, filters_base, kernel_size, save_path='weights_bkgr', **kw)    
